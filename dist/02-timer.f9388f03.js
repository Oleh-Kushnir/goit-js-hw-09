...
const fp = (0, _flatpickrDefault.default)(refs.input, {
    enableTime: true,
    time_24hr: true,
    defaultDate: new Date(),
    minuteIncrement: 1,
    onClose (selectedDates) {
        if (selectedDates[0] < Date.now()) return (0, _notiflix.Notify).failure("Please choose a date in the future");
        if (selectedDates[0] > Date.now()) return refs.btnSratr.disabled = false;
    }
});
class CountdownTimer {
    ...
    domUpdate({ days , hours , minutes , seconds  }) {
        ...
        if (refs.days.textContent === "00" && refs.hours.textContent === "00" && refs.mins.textContent === "00" && refs.secs.textContent === "00") (0, _notiflix.Notify).success("Timer is over");
        if (fp.selectedDates[0] < Date.now()) {
...
},{}]},["awmoY","1AWyZ"], "1AWyZ", "parcelRequired7c6")
